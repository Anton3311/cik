...
Before going into more details about Sea of Nodes, it is first important to mentation Single Static Assignment form, or SSA for short, which is a base for the Sea of Nodes.
...
Here, while tracking the current value of each variable, if we were to replace usages of `a` with its value, the expression tree would look like this:
...
To correctly replace usages, we would need to track every value that has ever been assigned to a variable, and when replacing a variable usage, we would need to pick the right value, based on where the actual expression is located in the code.

This quikly becomes complicated and messy. However there is a clear solution, which involves creating a new variable version on every assignment.
...
The above approach works flawlessly on linear code, however once branches get instroduced, further problems appear.
...
But then we come to the end of the if statement and return to the parent scope, that is where the problem is hiding. After returning to the parent scope, we somehow we need to decide which version of that variable to use. The if statement branch might run, or might not, therefore the new variable version might be created or might not be, which is fully based on the condition.

And that is where the phi nodes come into play. They are used to select different variable versions based on a condition.
...
With that part now covered, it is now possible explore the Sea of Nodes form, before going deeper into the details of how the compiler is implemented.


## Sea of Nodes

Sea of Nodes is built upon SSA, however traditional implementation of SSA have Control Flow Graph along side them. Sea of Nodes, however, merges these two structures into a single graph, turning CFG nodes into their own instructions, which are called `regions`.
...
However the lack of an explicit order is both a blessing and a curse, in some cases the compiler can reorder operations to speedup execution, in others however, it is neccessery to define an explicit order of operations to ensure correctness of the program.

One of such cases are function calls. For instance, if the program has two sebsequent calls to `printf`, we want them to happen in the same order they were defined in the source code, or else the output wouldn't be the one that was expected. This is one of many examples when the explicit serialization of operations must be used. 
...
For example, if we a have a call followed by a control instruction, the control depends on the call, and since this dependecy must be satisfied, the call will happen before the return.
...
Finally when lowering to machine code, the call at 4 will execute first, then call at 6 and finally the return.
